...
static int      __get_index(SimpleSet *set, const char *key, uint64_t hash,
                            uint64_t *index);
static int      __assign_node(SimpleSet *set, const char *key, uint64_t hash,
                              uint64_t index);
...
int set_intersection(SimpleSet *res, SimpleSet *s1, SimpleSet *s2) {
  if (res->used_nodes != 0) { return SET_OCCUPIED_ERROR; }
  // loop over both one of s1 and s2: get keys, check the other, and insert them
  // into res if it is
  uint64_t i;
  for (i = 0; i < s1->number_nodes; ++i) {
    if (s1->nodes[i] != NULL) {
      if (__set_contains(s2, s1->nodes[i]->_key, s1->nodes[i]->_hash) ==
          SET_TRUE) {
        __set_add(res, s1->nodes[i]->_key, s1->nodes[i]->_hash);
      }
    }
...
int set_symmetric_difference(SimpleSet *res, SimpleSet *s1, SimpleSet *s2) {
  ...
  for (i = 0; i < s2->number_nodes; ++i) {
    if (s2->nodes[i] != NULL) {
      if (__set_contains(s1, s2->nodes[i]->_key, s2->nodes[i]->_hash) !=
          SET_TRUE) {
        __set_add(res, s2->nodes[i]->_key, s2->nodes[i]->_hash);
      }
    }
...
static int __set_add(SimpleSet *set, const char *key, uint64_t hash) {
  ...
  if ((float)set->used_nodes / set->number_nodes > MAX_FULLNESS_PERCENT) {
    ...
    simple_set_node **tmp = (simple_set_node **)realloc(
        set->nodes, num_els * sizeof(simple_set_node *));
