...
typedef struct record {
  ...
  // profiling
  long unsigned int n_covered_prof_line;
  long unsigned int n_sglt_clusts_prof_line;
  long unsigned int n_singletons_prof_line;
  long unsigned int n_covered_prof_bb;
  long unsigned int n_sglt_clusts_prof_bb;
  long unsigned int n_singletons_prof_bb;
  long unsigned int n_covered_prof_intra;
  long unsigned int n_sglt_clusts_prof_intra;
  long unsigned int n_singletons_prof_intra;
  long unsigned int n_covered_prof_inter;
  long unsigned int n_sglt_clusts_prof_inter;
  long unsigned int n_singletons_prof_inter;
#ifndef IGNORE_FINDS
  // reset
...
typedef struct my_mutator {
  ...
  // stacktrace-based covman
  covmanager_t *covman_line;
  covmanager_t *covman_bb;
  covmanager_t *covman_intra;
  covmanager_t *covman_inter;
  ...
  Key2LinesCacheTable *key2lines;
  LineSetsTable       *linesets;
...
my_mutator_t *afl_custom_init(afl_state_t *afl, unsigned int seed) {
  ...
  // stack trace based residual risk
  data->covman_line = covmanager_init();
  data->covman_bb = covmanager_init();
  data->covman_intra = covmanager_init();
  data->covman_inter = covmanager_init();
#ifndef IGNORE_FINDS
  // reset estimator
  ...
  // check the number of basic blocks from environment variable "FT_BB_CNT"
  char *bb_cnt_str = getenv(ENV_BB_COUNT);
  if (bb_cnt_str) {
    afl->llvm_bb_map_size = atoi(bb_cnt_str);
    if (afl->llvm_bb_map_size <= 0) {
      FATAL("Invalid FT_BB_CNT value: %s", bb_cnt_str);
    }
    printf("Using FT_BB_CNT: %d\n", afl->llvm_bb_map_size);
  } else {
    FATAL(
        "FT_BB_CNT environment variable is not set. Please set it to the "
        "number of basic blocks in the target program.");
  }
  // initialize shared memory for the records
  create_shm(afl);

  // load bbinfo.json
  // get its path from env variable "ST_BBINFO_PATH"
  char *bbinfo_path = getenv(ENV_BBINFO_PATH);
  if (!bbinfo_path) {
    FATAL(
        "ST_BBINFO_PATH environment variable is not set. Please set it to the "
        "path of the bbinfo.json file.");
  }
  data->bbinfo = load_json(bbinfo_path);
  if (!data->bbinfo) { FATAL("Failed to load bbinfo JSON"); }

  // load mustexecinfo.json
  // get its path from env variable "ST_MUSTEXECINFO_PATH"
  char *mustexecinfo_path = getenv(ENV_MUSTEXECINFO_PATH);
  if (!mustexecinfo_path) {
    FATAL(
        "ST_MUSTEXECINFO_PATH environment variable is not set. Please set it "
        "to "
        "the path of the mustexecinfo.json file.");
  }
  data->mustexecinfo = load_json(mustexecinfo_path);
  if (!data->mustexecinfo) { FATAL("Failed to load mustexecinfo JSON"); }

  // initialize the key2lines cache
  data->key2lines = NULL;
  data->linesets = NULL;
...
void reset_entire_data(my_mutator_t *data) {
  ...
  // stacktrace-based residual risk
  destroy_covmanager(data->covman_line);
  data->covman_line = covmanager_init();
  destroy_covmanager(data->covman_bb);
  data->covman_bb = covmanager_init();
  destroy_covmanager(data->covman_intra);
  data->covman_intra = covmanager_init();
  destroy_covmanager(data->covman_inter);
  data->covman_inter = covmanager_init();
...
void afl_custom_post_run(my_mutator_t *data) {
  ...
  // stacktrace-based residual risk
  data->covman_line->n_execs++;
  data->covman_bb->n_execs++;
  data->covman_intra->n_execs++;
  data->covman_inter->n_execs++;
#ifndef IGNORE_FINDS
  ...
  SimpleSet *new_sglt_clust_line = (SimpleSet *)malloc(sizeof(SimpleSet));
  set_init(new_sglt_clust_line);
  SimpleSet *new_sglt_clust_bb = (SimpleSet *)malloc(sizeof(SimpleSet));
  set_init(new_sglt_clust_bb);
  SimpleSet *new_sglt_clust_intra = (SimpleSet *)malloc(sizeof(SimpleSet));
  set_init(new_sglt_clust_intra);
  SimpleSet *new_sglt_clust_inter = (SimpleSet *)malloc(sizeof(SimpleSet));
  set_init(new_sglt_clust_inter);
#ifndef IGNORE_FINDS
  ...
  // printf("covered this time: [");
  for (i = 0; i < data->afl->llvm_bb_map_size; i++) {
    if (read_shm_index(data->afl, i)) {  // idx i is covered
      // printf("%d, ", i);
  ...
  // printf("] \n"); read shared memory for the profiling and update the
  // coverage managers for the profiling const char *key_prof =
  // idx_to_str(read_shm_profile(data->afl));
  int key_prof = read_shm_profile(data->afl);
  // printf("key_prof: %d\n", key_prof);
  const char *line_prof =
      get_line_prof(&data->key2lines, data->bbinfo, key_prof);
  // printf("line_prof: %s\n", line_prof);
  const char **bb_prof =
      get_bb_prof(&data->linesets, data->mustexecinfo, line_prof);
  // printf("bb_prof: ");
  // for (i = 0; bb_prof[i] != NULL; i++) {
  //   printf("%s, ", bb_prof[i]);
  // }
  // printf("\n");
  const char **intra_prof =
      get_intra_prof(&data->linesets, data->mustexecinfo, line_prof);
  // printf("intra_prof: ");
  // for (i = 0; intra_prof[i] != NULL; i++) {
  //   printf("%s, ", intra_prof[i]);
  // }
  // printf("\n");
  const char **inter_prof =
      get_inter_prof(&data->linesets, data->mustexecinfo, line_prof);
  // printf("inter_prof: ");
  // for (i = 0; inter_prof[i] != NULL; i++) {
  //   printf("%s, ", inter_prof[i]);
  // }
  // printf("\n");
  // update covmanagers for profiling
  // is_update = update_covmanager(data->covman_bb, key_prof, new_sglt_clust_bb)
  // ||
  //             is_update;
  is_update =
      update_covmanager(data->covman_line, line_prof, new_sglt_clust_line) ||
      is_update;
  for (i = 0; bb_prof[i] != NULL; i++) {
    is_update =
        update_covmanager(data->covman_bb, bb_prof[i], new_sglt_clust_bb) ||
        is_update;
  }
  for (i = 0; intra_prof[i] != NULL; i++) {
    is_update = update_covmanager(data->covman_intra, intra_prof[i],
                                  new_sglt_clust_intra) ||
                is_update;
  }
  for (i = 0; inter_prof[i] != NULL; i++) {
    is_update = update_covmanager(data->covman_inter, inter_prof[i],
                                  new_sglt_clust_inter) ||
                is_update;
  }
  // free the profiling data
  // if (bb_prof) { free(bb_prof); }
  // if (intra_prof) { free(intra_prof); }
  // if (inter_prof) { free(inter_prof); }

  // if there is new singleton cluster, add it to sglt_clusts
  update_singleton_clusters(data->covman_total, new_sglt_clust_total);
  update_singleton_clusters(data->covman_line, new_sglt_clust_line);
  update_singleton_clusters(data->covman_bb, new_sglt_clust_bb);
  update_singleton_clusters(data->covman_intra, new_sglt_clust_intra);
  update_singleton_clusters(data->covman_inter, new_sglt_clust_inter);
#ifndef IGNORE_FINDS
  ...
  if (add_new_record || data->force_save) {
    ...
    new_record->n_covered_prof_line =
        set_length(data->covman_line->covered_prev);
    new_record->n_sglt_clusts_prof_line = data->covman_line->n_sglt_clusts;
    new_record->n_singletons_prof_line =
        set_length(data->covman_line->singletons);

    new_record->n_covered_prof_bb = set_length(data->covman_bb->covered_prev);
    new_record->n_sglt_clusts_prof_bb = data->covman_bb->n_sglt_clusts;
    new_record->n_singletons_prof_bb = set_length(data->covman_bb->singletons);

    new_record->n_covered_prof_intra =
        set_length(data->covman_intra->covered_prev);
    new_record->n_sglt_clusts_prof_intra = data->covman_intra->n_sglt_clusts;
    new_record->n_singletons_prof_intra =
        set_length(data->covman_intra->singletons);

    new_record->n_covered_prof_inter =
        set_length(data->covman_inter->covered_prev);
    new_record->n_sglt_clusts_prof_inter = data->covman_inter->n_sglt_clusts;
    new_record->n_singletons_prof_inter =
        set_length(data->covman_inter->singletons);
...
void write_header(FILE *f, bool for_done_records) {
  ...
  fprintf(f,
          ...
          "#coveredLine, #singletonsLine, #sglt_clustsLine, "
          "#coveredBB, #singletonsBB, #sglt_clustsBB, "
          "#coveredIntra, #singletonsIntra, #sglt_clustsIntra, "
          "#coveredInter, #singletonsInter, #sglt_clustsInter, "
          "#foundnew, done");
#else
  fprintf(f,
...
void write_row(FILE *f, my_mutator_t *data, record_t *cur,
  ...
  fprintf(f,
          ...
          cur->n_covered_prof_line, cur->n_singletons_prof_line,
          cur->n_sglt_clusts_prof_line, cur->n_covered_prof_bb,
          cur->n_singletons_prof_bb, cur->n_sglt_clusts_prof_bb,
          cur->n_covered_prof_intra, cur->n_singletons_prof_intra,
          cur->n_sglt_clusts_prof_intra, cur->n_covered_prof_inter,
          cur->n_singletons_prof_inter, cur->n_sglt_clusts_prof_inter,
          cur->n_found_new,
...
void afl_custom_deinit(my_mutator_t *data) {
  ...
  free(data->item2man);
#endif
  cleanup_shm(data->afl);

  if (data->bbinfo) { cJSON_Delete(data->bbinfo); }
  if (data->mustexecinfo) { cJSON_Delete(data->mustexecinfo); }
  clear_line_cache(data->key2lines);
  clear_linesets_cache(data->linesets);
